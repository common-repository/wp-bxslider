...
Modified for use in wpslider
*/


jQuery.fn.wpSlider = function(options){
	...
	return this.each(function(){
		...
		if(o.alignment == 'horizontal'){
			...
			$kids.each(function(){
				...
				if(jQuery(this).width() > w){h = jQuery(this).width();}					
			});
			if ( o.set_w > 0 ) {
				w = o.set_w; 
			}
			...
			$this.children().css({'float':'left',	'listStyle':'none', 'marginRight':o.margin, 'width': w-o.margin});
			...
			$this.children().css({'height':h, 'listStyle':'none', 'marginBottom':o.margin, 'width': w});
		...
		if(o.pager && !o.ticker){
			...
			if(!o.pager_short){
				...
				$kids.each(function(index){
					$a = jQuery('<a class="pager_link_'+index+' href="#">'+(index+1)+'</a>');
		...
		if(o.auto && !o.ticker){
			...
			if(o.auto_controls){
				...
				$this.parent().siblings('.auto_controls').find('a').click(function(){
					if(playing){clearInterval(t); jQuery(this).html(o.start_text); playing=false;}
					else{
						t = setInterval(function(){if(o.auto_direction == 'next'){move_slide(++current);}else{move_slide(--current);}}, o.pause);
						jQuery(this).html(o.stop_text);
						playing=true;
					}
					return false;
				});
		...
		if(o.ticker){
			...
			if(o.ticker_controls){
				...
				$this.parent().siblings('.ticker_controls').find('a').click(function(){
					if(tick_play){$this.stop(); jQuery(this).html(o.start_text); tick_play=false;}
					else{
						is_working = false;
						jQuery(this).html(o.stop_text)
						tick_slide();
